...
ist_time = datetime.now(ZoneInfo("Asia/Kolkata")).strftime('%d-%b-%Y %I:%M:%S %p')
...
async def start(bot, m: Message):
    ...
    for step in steps:
        ...
        await start_message.edit(step)

    # Final authenticated message
    await asyncio.sleep(1)
    await start_message.edit(
        f"🎉 **Welcome Agent {user}**\n\n"
        f"✅ You are now connected to the STRANGER Uploader Terminal.\n"
        f"💡 Type `/help` to view available commands.\n"
        f"👨‍💻 Authenticated as: `PREMIUM_USER`\n"
        f"🔗 [Contact Support](https://t.me/Strangerboy27_bot_strangerboy)\n",
    ...
    await start_message.edit_text(
    f"💻 [ ACCESS DENIED ]\n\n"
    f"👤 Welcome `{m.from_user.first_name}` to **STRANGER DRM BOT** 🔐\n\n"
    f"🧠 Initializing public access module...\n"
    f"📦 Feature Access: `NON-PREMIUM`\n\n"
    f"🔍 Supported Targets:\n"
    ...
    f"• 📦 APPX Enc PDFs\n"
    f"• 📺 Vimeo / Brightcove\n"
    f"• 🛰️ VisionIAS / Utkarsh (Video + PDF)\n"
    f"• 🔓 MPD URLs (if KEY is known)</blockquote>\n\n"
    f"⚠️ You are currently on a **FREE** Plan.\n"
    f"<blockquote>💵 Monthly Plan: `FREE - LIMITED ACCESS`</blockquote>\n\n"
    f"💬 To upgrade your plan, contact admin:\n"
    f"[STRANGER BOYS SUPPORT](https://t.me/Strangerboy27_bot_strangerboy)",
    disable_web_page_preview=True,
...
async def upgrade_command(client, message: Message):
    await message.reply_text(
        ...
        f"Here's what you'll unlock with PREMIUM access:\n\n"
        f"<blockquote>• 🔐 Classplus DRM/ND\n"
        f"• 📚 APPX Zip/Enc PDF\n"
        f"• 🧑‍🏫 PW + Khan + StudyIQ DRM\n"
        f"• 📡 Vimeo / Brightcove\n"
        f"• 🎓 Utkarsh + VisionIAS\n"
        f"• 🔓 MPD URL Decryption</blockquote>\n\n"
        f"📢 **Monthly Plan:** `FREE (Limited)`\n\n"
        f"📩 Contact Admin → [STRANGER BOYS](https://t.me/Strangerboy27_bot_strangerboy)",
        disable_web_page_preview=True,
        reply_markup=BUTTONSCONTACT
    )
...
async def id_command(client, message: Message):
    await message.reply_text(
        f"💾 [ CHAT IDENTIFICATION ]\n\n"
        f"<blockquote>🆔 Your Chat ID:\n`{message.chat.id}`</blockquote>"
    )
@bot.on_message(filters.private & filters.command(["info"]))
async def info(bot: Client, update: Message):
    text = (
        f"╭─[ STRANGER USER INFO ]─────────────╮\n"
        f"├ 🔹 Name      : `{update.from_user.first_name} {update.from_user.last_name or ''}`\n"
        f"├ 🔹 Username  : @{update.from_user.username or 'N/A'}\n"
...
async def txt_handler(client: Client, m: Message):
    await bot.send_message(m.chat.id, text= (
        ...
        f"╰━━━━━━━✦✧✦━━━━━━━╯\n"
        f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n" 
        f"🧠 [ STRANGER DRM TOOLKIT ] 🧠\n"
        f"🔒 Status: [ENCRYPTED]\n"
        f"💻 Engine: v1.7.4 | ⚙️ Mode: Active\n"
        f"🛰️ Network: Secure | 📡 Signal Strength: Excellent\n"
        f"🎯 Target Locked: DRM Bypass Initialized\n"
        f"🧬 Identity: Stranger | 🕶️ Access Level: ADMIN\n"
        f"🔥 Launch Protocols Armed | ✅ SYSTEM READY\n"
        f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n" 
        f"╭━━━━━━━━━━━━━ ❀° ━━━╮\n"
        f"┣⪼📌 𝗠𝗮𝗶𝗻 𝗙𝗲𝗮𝘁𝘂𝗿𝗲𝘀:\n\n"  
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            try:
                cc = f'⌁⌁⌁ [ 👾 𝗘𝗫𝗧𝗥𝗔𝗖𝗧𝗘𝗗 𝗕𝗬 STRANGER_BOT ](https://i.ibb.co/ccV44ZRS/STRANGER-BOY.jpg) ⌁⌁⌁\n[🎥]Vid Id : {str(count).zfill(3)}\n🔐 **File:**  `{name1} [{res}p] .mkv`\n> ```\n[:: CLASSIFIED VIDEO STREAM ::]\n{b_name}\n```\n"\n👤 Uploader ➤ [{CR}](https://i.ibb.co/7xm7cXyg/STRANGER-BOY.jpg)\n📅 Time     : {ist_time} IST\n\n⌬ [ ⚠️ 𝗦𝗘𝗖𝗨𝗥𝗘𝗗 𝗕𝗬 STRANGER CORE ⚠️ ](https://envs.sh/49s.mp4) ⌬\n'
                cc1 = f'⌁⌁⌁ [ 👾 𝗘𝗫𝗧𝗥𝗔𝗖𝗧𝗘𝗗 𝗕𝗬 STRANGER_BOT ](https://i.ibb.co/ccV44ZRS/STRANGER-BOY.jpg) ⌁⌁⌁\n[📕]Pdf Id : {str(count).zfill(3)}\n🔐 **File:**  `{name1} .pdf`\n> ```\n[:: INTEL DOCUMENT DUMP ::]\n{b_name}\n```\n"\n👤 Declassified by ➤ [{CR}](https://i.ibb.co/ZRvCxd5g/STRANGER-BOY.jpg)\n📅 Time     : {ist_time} IST\n\n⌬ [ ⚠️ 𝗦𝗘𝗖𝗨𝗥𝗘𝗗 𝗕𝗬 STRANGER CORE ⚠️ ](https://envs.sh/49s.mp4) ⌬\n'
                cczip = f'⌁⌁⌁ [ 👾 𝗘𝗫𝗧𝗥𝗔𝗖𝗧𝗘𝗗 𝗕𝗬 STRANGER_BOT ](https://i.ibb.co/ccV44ZRS/STRANGER-BOY.jpg) ⌁⌁⌁\n[📁]Zip Id : {str(count).zfill(3)}\n🔐 **File:**  `{name1} .zip`\n> ```\n[:: ENCRYPTED ARCHIVE ::]\n{b_name}\n```\n"\n👤 Extractor ➤ [{CR}](https://i.ibb.co/ccV44ZRS/STRANGER-BOY.jpg)\n📅 Time     : {ist_time} IST\n\n⌬ [ ⚠️ 𝗦𝗘𝗖𝗨𝗥𝗘𝗗 𝗕𝗬 STRANGER CORE ⚠️ ](https://envs.sh/49s.mp4) ⌬\n' 
                ccimg = f'⌁⌁⌁ [ 👾 𝗘𝗫𝗧𝗥𝗔𝗖𝗧𝗘𝗗 𝗕𝗬 STRANGER_BOT ](https://i.ibb.co/ccV44ZRS/STRANGER-BOY.jpg) ⌁⌁⌁\n[🖼️]Img Id : {str(count).zfill(3)}\n🔐 **File:**  `{name1} .jpg`\n"> ```\n[:: VISUAL EXFILTRATION ::]\n{b_name}\n```\n"\n👤 Rendered by ➤ [{CR}](https://i.ibb.co/7xm7cXyg/STRANGER-BOY.jpg)\n📅 Time     : {ist_time} IST\n\n⌬ [ ⚠️ 𝗦𝗘𝗖𝗨𝗥𝗘𝗗 𝗕𝗬 STRANGER CORE ⚠️ ](https://envs.sh/49s.mp4) ⌬\n'
                ccm = f'⌁⌁⌁ [ 👾 𝗘𝗫𝗧𝗥𝗔𝗖𝗧𝗘𝗗 𝗕𝗬 STRANGER_BOT ](https://i.ibb.co/ccV44ZRS/STRANGER-BOY.jpg) ⌁⌁⌁\n[🎵]Audio Id : {str(count).zfill(3)}\n**🔐 **File:**  `{name1} .mp3`\n"> ```\n[:: SECURE TRANSMISSION AUDIO ::]\n{b_name}\n```\n"\n👤 Routed by ➤ [{CR}](https://i.ibb.co/ZRvCxd5g/STRANGER-BOY.jpg)\n📅 Time     : {ist_time} IST\n\n⌬ [ ⚠️ 𝗦𝗘𝗖𝗨𝗥𝗘𝗗 𝗕𝗬 STRANGER CORE ⚠️ ](https://envs.sh/49s.mp4) ⌬\n'
                cchtml = f'⌁⌁⌁ [ 👾 𝗘𝗫𝗧𝗥𝗔𝗖𝗧𝗘𝗗 𝗕𝗬 STRANGER_BOT ](https://i.ibb.co/ccV44ZRS/STRANGER-BOY.jpg) ⌁⌁⌁\n[🌐]Html Id : {str(count).zfill(3)}\n🔐 **File:**  `{name1} .html`\n"> ```\n[:: WEB INTEL SNAPSHOT ::]\n{b_name}\n```\n"\n👤 Surfed by ➤ [{CR}](https://i.ibb.co/ccV44ZRS/STRANGER-BOY.jpg)\n📅 Time     : {ist_time} IST\n\n⌬ [ ⚠️ 𝗦𝗘𝗖𝗨𝗥𝗘𝗗 𝗕𝗬 STRANGER CORE ⚠️ ](https://envs.sh/49s.mp4) ⌬\n'
